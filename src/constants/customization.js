...
export const FONT_PREVIEW_CLASSES = [
  { value: 'cm-font-elegant-script', label: 'Elegant Script' },
  { value: 'cm-font-luxury-serif', label: 'Luxury Serif' },
  { value: 'cm-font-modern-sans', label: 'Modern Sans' },
  { value: 'cm-font-classic-roman', label: 'Classic Roman' },
  { value: 'cm-font-minimal-line', label: 'Minimal Line' },
  { value: 'cm-font-bold-signature', label: 'Bold Signature' },
];
...
export const getOptionIds = (entries = []) =>
  (Array.isArray(entries) ? entries : []).map(getOptionId).filter(Boolean);
...
/**
 * Storefront PDP resolver: only sections the product/admin actually configured
 * as enabled are shown. Missing keys stay disabled (unlike mergeCustomizationOptions).
 */
export const resolveProductCustomizationOptions = (options = {}) => {
  const defaults = getDefaultCustomizationOptions();
  const normalized = normalizeCustomizationOptionsShape(options);
  const resolved = {};

  Object.keys(defaults).forEach((key) => {
    const productSection = normalized[key];

    if (!productSection || typeof productSection !== 'object' || Array.isArray(productSection)) {
      resolved[key] = { ...defaults[key], enabled: false, options: [] };
      return;
    }
    ...
    resolved[key] = {
      ...defaults[key],
      ...productSection,
      enabled: Boolean(productSection.enabled),
    };

    if (LIST_OPTION_KEYS.includes(key)) {
      resolved[key].options = Array.isArray(productSection.options)
        ? productSection.options.map((entry) => ({ ...entry }))
        : [];
    }
  });
